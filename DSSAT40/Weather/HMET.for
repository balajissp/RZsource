...
!     TS defined in ModuleDefs.for
     ...
     &  TAVG,TDAY,TDEW,TGRO(TS),TGROAV,TGRODY,TINCR,TMAX,TMIN,
     &  RADHR(TS),RH,RHUMHR(TS),VPSAT,WINDAV,WINDHT,WINDSP,
     &  WINDHR(TS),XLAT
...
C       Calculate sun angles and hourly weather variables.
     ...
     &    PARHR(H))                                       !Output
      ...
      IF (PAR .LE. 0.) THEN
        PAR = 2.0 * SRAD   !converts from MJ/m2/d to mole (quanta)/m2/d
      ENDIF
...
!-----------------------------------------------------------------------
      IMPLICIT NONE
      REAL HS,PAR,PARFAC,PARHR,RADHR,SNDN,SNUP,SRAD
...
C         PARFAC = (0.43*(1.0-FRDIFR)+0.57*FRDIFR) * PARQC
          PARFAC = 2.0
c          PARHR = RADHR * PARFAC
          PARHR = 0.0   !set to zero and let RZMAIN.FOR to set to double radiation
        ENDIF
