...
!     From main program CHP 08/27/01
      SPGROF = (.167*(1.0-.82*EXP(-.065*SPRLAP)))/.0344    !sprlap=cm sprout length at planting
!      S1     = SIN (XLAT*0.01745)
!      C1     = COS (XLAT*0.01745)
...
C-----------------------------------------------------------------------
        CASE (7)
          ...
          !
          NDAS = NDAS + 1
c          IF (SPRLAP .EQ. 0.0) THEN  !Liwang Ma, 3-4-2008
...
C-----------------------------------------------------------------------
      ...
      INTEGER ERR, FOUND, LINC, LNUM, IEMRG, iemrgr
...
c          G3 = PLANTVAR% G3
	    PLANTS = PLANTVAR% PLTPOP      ! need to check with Gerrit Hoogenboom, Liwang Ma
          if ((control % IEMRG .eq. 0)
     +        .or.(control % IEMRG .eq. -99))then
              iemrg = -99
          else
          call YR_DOY(control % yrplt, YR,IemRGr)
          IEMRG  = control % IEMRG + iemrgr    ! check Liwang Ma
          endif
          SPRLAP = PLANTVAR% sprlap   !2.0                     ! check Liwang Ma
...
c        CALL ERROR(SECTION, 42, FILEIO, LNUM)
c      ELSE
c        READ (LUNIO,'(15X,I3,1X,F5.1,31X,F5.1,25X,F5.0)', IOSTAT=ERR) 
c     &            IEMRG, PLANTS, SDEPTH, SPRLAP ; LNUM = LNUM + 1
c        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILEIO,LNUM)
c      ENDIF
...
c          READ (LUNIO,'(55X,2F6.0)', IOSTAT=ERR) P2, TC
c          LNUM = LNUM + 1
c          IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILEIO,LNUM)
c        ENDIF
c      ENDIF
