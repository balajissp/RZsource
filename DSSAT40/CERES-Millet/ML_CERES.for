...
C----------------------------------------------------------------------
     ...
     &    BWAH, SDWTAH,rootn*pltpop*10.0,RTWT*PLTPOP*10.0)!Output
...
C--------------------------------------------------------------------
     ...
     & VANC,VMNC,TLAG1, SENESCE, MPLA, TPLA, MPLAG, TPLAG)
      ...
      !---------------------------------------------------------------
          !IF (ISWDIS.EQ.'Y') THEN
C OUT BY LIWANG MA, RZWQM-DSSAT
C          CALL PEST(CONTROL, ISWITCH, 
C     &    AREALF, CLW, CSW, LAGSD, LNGPEG, NR2, CARBO,    !Input
C     &    PHTIM, PLTPOP, RTWTO, SLA, SLDOT, SOILPROP,     !Input
C     &    SSDOT, STMWTO, TOPWT, WLFDOT, WTLF, YRPLT,      !Input
...
C     %                   P1,P2O,P2R,P5,G1,G4,PHINT
!CHP 1800          FORMAT (A6,1X,A16,1X,A6,1X,F6.1,F6.3,2(F6.1),3(F6.2))    
C1800          FORMAT (A6,1X,A16,1X,A6,1X,7F6.0)    
C              LNUM = LNUM + 1
C              IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILEIO,LNUM)
C          ENDIF
...
C----------------------------------------------------------------------
     ...
     &    BWAH, SDWTAH,rootn*pltpop*10.0,RTWT*PLTPOP*10.0)!Output
        ...
        CALL HRes_Ceres(CONTROL,SOILPROP,
