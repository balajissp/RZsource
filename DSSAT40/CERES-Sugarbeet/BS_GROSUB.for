C======================================================================
C  BS_GROSUB, Subroutine
C
C  Sugarbeet growth routine
C----------------------------------------------------------------------
C  Revision history
C
C                 Written
C  04/15/2015 Model developed by Mohammad J Anar and Zhulu Lin based on CERES beet model developed by Leviel et al. (2000)     
C----------------------------------------------------------------------
C
C  Called : Sugarbeet
C
...
C----------------------------------------------------------------------
      ...
      SUBROUTINE BS_GROSUB (NOUTDO,ISWNIT,ISWWAT,IDETO,DYNAMIC,
...
C----------------------------------------------------------------------
       TRWUP1=TRWUP
C RESET LAI 12-16-2018, NOT SURE THIS IS CORRECT AS LFWT IS FROM MAIZE
...
!          CSD2   = 0.0
          ...
          IF (ISWNIT .NE. 'N') THEN
             CALL BS_NFACTO(DYNAMIC,ISTAGE,TANC,TCNP,TMNC,
          ...
          IF(ISWWAT.NE.'N') THEN
             ...
             IF (EOP .GT. 0.0) THEN
                ...
                IF (EP1 .GE. TRWUP) THEN
                  SWFAC = TRWUP / EP1
                ENDIF
cc          IF (ISTRESS.EQ.4) CALL HrootStress(TURFAC,SWFAC,EOP)
...
!              PLA = PLA + PLAG 
          ...
          !-------------------------------------------------------------
          !         ISTAGE = 2 (End of Juvenile to Pennicle Initiation)
          !-------------------------------------------------------------
...
!      --------------------------------------------------------------------
!         ISTAGE = 4 (Beginning of effective growth period)
!      --------------------------------------------------------------------
...
!          WTNUP = WTNUP + TRNU*PLTPOP  
              !Total N uptake, g/m2  Moved to BS_roots.for
...
c        STOVER  = BIOMAS-YIELD !RMarquez - above ground biomass is tiny compared to yield, always will be negative.
        YIELDB  = YIELD/62.8         
...
C----------------------------------------------------------------------
 ...
 2800 FORMAT (2X,'Crop failure growth due to cold stress on DAP---',i4)
 2801 FORMAT (2X,'Crop failure growth due to water stress on DAP---',i4)
...
! HI          Ratio of Root yield to total plant biomass 
! I           !Counter
