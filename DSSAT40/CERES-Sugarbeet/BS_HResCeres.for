...
C=======================================================================
      ...
      SUBROUTINE BS_HRes_Ceres(CONTROL,SOILPROP,
...
! Liwang Ma, 9-17-2007 to compute residue
c      IF (CROP /= 'FA' .AND. INDEX('QF',CONTROL%RNMODE) > 0) THEN

C-----------------------------------------------------------------------
!       By-product not harvested:
        TOPRES = AMAX1(0.0, STOVWT * PLTPOP * 10.)
!        kg/ha               g/pl    pl/m2              

!       Grain not harvested
        GRNRES = AMAX1(0.0, GRNWT * PLTPOP * 10.)
!        kg/ha               g/pl   pl/m2  
            
!       Add cob (chaff) weight -- 100%.    !not 100% now based on Sasi's suggestion 8-6-2008
...
!       Total root residues (whole profile) equal root weight 
        TRTRES = RTWT * PLTPOP * 10. * (1. - HARVFRAC(1))             !kg/ha
!             g/plant * plants/m2

!       N in root residues.
        TRTRESE(N) = ROOTN * PLTPOP * 10. * (1. - HARVFRAC(1))      !kg/ha
!                 g[N]/plant * plants/m2  
...
c        CALL IPSOIL (CONTROL,'RE001',                   !Input
c     &    DMINR, DSNC, PRCEL, PRCHO, PRLIG,             !Output
c     &    RCN, RDCEL, RDCHO, RDLIG)                     !Output
c
        !Multiply biomass array by % lignin to get lignin component
c        HResLig = HResWt * PRLIG / 100.
...
!       Senesced leaf and stem may be harvested for byproduct.
     ...
     &                    * (1. - HARVFRAC(2))
        ...
        ENDDO
c Liwang Ma, 9-17-2007 to compute residue
c      ENDIF   !Crop .NE. 'FA'
C-----------------------------------------------------------------------
