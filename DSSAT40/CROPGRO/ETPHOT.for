...
C-----------------------------------------------------------------------
     ...
     &  XLMAXT(6),XSW(NL,3),YLMAXT(6),YSCOND(NL,3),YSHCAP(NL,3),TMIN
      ...
      REAL, DIMENSION(NL) :: BD, DUL, SAT2, RWU2, DUL2, RLV2
...
C MEPHO    MEEVP
C -----  ----------
C  'C'   'P' or 'R'  PHOTO used for photosynthesis, WATBAL used for ET
C  'L'   'P' or 'R'  ETPHOT used for photosynthesis, WATBAL used for ET
C  'L'       'Z'     ETPHOT used for both photosynthsis and ET
C========================================================================
...
!     FINAL
!***********************************************************************
      ELSEIF (DYNAMIC .EQ. FINAL .OR. DYNAMIC .EQ. OUTPUT) THEN
!-----------------------------------------------------------------------
        IF (MEPHO .EQ. 'L') THEN
                CALL OpETPhot(CONTROL, ISWITCH,
     &        PCINPD, PG, PGNOON, PCINPN, SLWSLN, SLWSHN,
...
!     ------------------------------------------------------------------
      ...
      CHARACTER BLANK*1,ERRKEY*6,FILEC*12,FILECC*255,FILEIO*30,
     &  PATHCR*255,SECTION*6
      CHARACTER*80 C80
     ...
     &  KSOIL,LFANGB,LFANGD(3),LL(NL),LL2(NL),LLE,LWIDTH,N,PALBW,
...
C=======================================================================
     ...
     &  XLMAXT, YLMAXT, PHTHRS10)                         !Output
      ...
      CHARACTER BLANK*1,ERRKEY*6,FILEC*12,FILECC*255,FILEIO*30,
     &  PATHCR*255,SECTION*6,TYPPGL*3,TYPPGN*3
      CHARACTER*80 C80
...
C  MODIFED BY LIWANG MA, RZWQM-DSSAT
          ...
          FILEC = CONTROL % FILES   !SOMETIMES, S AND C ARE REVISED!!!
C          FILEE = CONTROL % FILEE
C          FILES = CONTROL % FILES
C          PATHSR = CONTROL % PATHSR
          PATHCR = CONTROL % PATHSR  !SOMETIMES, S AND C ARE REVISED!!!
C          PATHER = CONTROL % PATHER
	    ...
	    PLTPOP = PLANTVAR% PLTPOP
          ...
          AZIR = PLANTVAR% AZIR
          PHTHRS10 = PLANTVAR% PHTHRS(10)
          LMXREF = PLANTVAR% LFMAX            !ASK GERRIT HOOGENBOOM
...
c      READ (LUNIO,'(////15X,A,1X,A)',IOSTAT=ERRNUM) FILEC,PATHCR
c      IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,5)
c Liwang Ma      PATHL  = INDEX(PATHCR,BLANK)
      PATHL  = Ipath(PATHCR)
      IF (PATHL .LE. 1) THEN
        FILECC = FILEC
...
C      READ(LUNIO,'(24X,F6.0,12X,2F6.0)',IOSTAT=ERRNUM)PLTPOP,ROWSPC,AZIR
C      LNUM = LNUM + 1
C      IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM+1)
...
C      READ(LUNIO,'(60X,F6.0,6X,F6.0)',IOSTAT=ERRNUM) PHTHRS10,LMXREF
C      LNUM = LNUM + 1
C      IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM+1)
...
C=======================================================================
      SUBROUTINE GetPutPG(GETPUT, AGEFAC, PG)
...
!-----------------------------------------------------------------------
      IF (GETPUT .EQ. 'GET') THEN
        !Retreive data
        PG     = PGData % PG
        AGEFAC = PGData % AGEFAC
      ELSEIF (GETPUT .EQ. 'PUT') THEN
        !Store data
        PGData % PG     = PG
        PGData % AGEFAC = AGEFAC
      ENDIF

      RETURN
      END SUBROUTINE GetPutPG
C=======================================================================

C=======================================================================
      SUBROUTINE GetPutPlant(GETPUT,
     &  CANHT, CANWH, DXR57, EXCESS, NR5, PLTPOP, RNITP, SLAAD, XPOD)
...
!-----------------------------------------------------------------------
      ...
      CHARACTER*3 GETPUT
      INTEGER NR5
      REAL CANHT, CANWH, DXR57, EXCESS, PLTPOP, RNITP, SLAAD, XPOD
     ...
     &    PLTPOP, RNITP, SLAAD, XPOD
        INTEGER NR5
      END TYPE PlantType
      TYPE (PlantType) PlantData

!-----------------------------------------------------------------------
      IF (GETPUT .EQ. 'GET') THEN
        !Retreive data
        CANHT  = PlantData % CANHT
        CANWH  = PlantData % CANWH
        DXR57  = PlantData % DXR57
        EXCESS = PlantData % EXCESS
        NR5    = PlantData % NR5
        PLTPOP = PlantData % PLTPOP
        RNITP  = PlantData % RNITP
        SLAAD  = PlantData % SLAAD
        XPOD   = PlantData % XPOD
      ELSEIF (GETPUT .EQ. 'PUT') THEN
        !Store data
        PlantData % CANHT  = CANHT
        PlantData % CANWH  = CANWH
        PlantData % DXR57  = DXR57
        PlantData % EXCESS = EXCESS
        PlantData % NR5    = NR5
        PlantData % PLTPOP = PLTPOP
        PlantData % RNITP  = RNITP
        PlantData % SLAAD  = SLAAD
        PlantData % XPOD   = XPOD
      ENDIF

      RETURN
      END SUBROUTINE GetPutPlant
C=======================================================================
