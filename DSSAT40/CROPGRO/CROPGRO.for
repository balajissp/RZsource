...
C=======================================================================
     ...
     &    STGDOY, UNH4, UNO3, XHLAI, XLAI,NVEG0,RTWT,STMWT,
     &    NFIXN,TOPWT,WTLF,PCNL,PCNST,PCNRT,PCNSD,RSTAGE,SDWT,TRNU,
     &    wtnfx,pronod,nodgr,rtdep,swfac,turfac,SRDOT)
     &          !Output
...
!     Senescence variables computed for soil N routines.
     ...
     &    WTLF, WSHIDT, WLIDOT
      REAL XPOD, XFRT, XHLAI, XLAI,pronod
...
!     Species-dependant variables exported to SPAM module:
      REAL EORATIO, KCAN, KEP, KSEVAP, KTRANS, PORMIN, RWUMX, RWUEP1
...
!-----------------------------------------------------------------------
     ...
     &    TMIN, TURFAC, XPOD, YRPLT,                      !Input
     &    DRPP, DTX, DXR57, FRACDN, MDATE, NDLEAF,        !Output
     &    NDSET, NR1, NR2, NR5, NR7, NVEG0, PHTHRS,       !Output
     &    RSTAGE, RVSTGE, STGDOY, TDUMX, TDUMX2, VSTAGE,  !Output
     &    YREMRG,YRNR1, YRNR2, YRNR3, YRNR5, YRNR7)       !Output
...
!-----------------------------------------------------------------------
     ...
     &  WTSD, WTSHE, XPOD, YRDOY, NVEG0, NR1, NR2, NR5,   !Input
     &  NR7, YRSIM,                                       !Input
...
C-----------------------------------------------------------------------
        CALL INCOMP(RUNINIT, 
     &    ECONO, FILECC, FILEGC, FRLF, FRRT, FRSTM,       !Input
...
!-----------------------------------------------------------------------
        !IF (ISWNIT .EQ. 'Y') THEN
          CALL NUPTAK(
     &      BD, DLAYR, DUL, FILECC, LL, NDMSDR, NDMTOT,   !Input
...
!-----------------------------------------------------------------------
        CALL VEGGR(
     &    AGRLF,  AGRRT,  AGRSTM, CMINEP,  CSAVEV,        !
     &    DTX,    DXR57,  ECONO,  FILECC,  FILEGC, FNINL, !
     &    FNINR,  FNINS,  NAVL,   NDMNEW,  NDMOLD,        !
     &    NFIXN,  NMINEA, NR1, PAR,  PCH2O,  PG,          !Input
     &    PGAVL,  ROWSPC, RVSTGE, STMWT,   TGRO,          !
     &    TRNU,   TURFAC, VSTAGE, WCRLF,  WCRRT,   WCRSH, !
     &    WCRST,  WTLF,   XLAI,   YRDOY,  YREMRG,  YRSIM, !

     &    AGRVG,  FRLF,   FRRT,   FRSTM,                  !Input/Output
...
C-----------------------------------------------------------------------
        CALL SENES(
     &    FILECC, CLW, DTX, NR7, NRUSLF, PAR, RHOL,       !Input
     &    SLAAD, STMWT, SWFAC, VSTAGE, WTLF, XLAI,        !Input
     &    YRDOY, YRSIM,                                   !Input
...
!-----------------------------------------------------------------------
     ...
     &  NPLTD, NRUSLF, NRUSRT, NRUSSH, NRUSST, POTCAR,    !Input
     &  POTLIP, PPLTD, SDIDOT, SDPROR,                    !Input
...
!     Initialize Overview.out file.
     ...
     &    YRNR7, YRPLT,                                   !Input
     &    SDWTAH,RTWT*PCNRT/100.0*10.0,RTWT*10.0)         !Output
...
!       Calculate daily water stess factors (from SWFACS)
        ...
        IF (EOP .GT. 0.001) THEN
          ...
          IF (EP1 .GE. TRWUP) THEN
            SWFAC = TRWUP / EP1
          ENDIF
cc          IF (ISTRESS.EQ.4) CALL HrootStress(TURFAC,SWFAC,EOP)  !not used anymore.
        ENDIF
      ENDIF
...
!-----------------------------------------------------------------------
      ELSE IF (DYNAMIC .EQ. OUTPUT .OR. DYNAMIC .EQ. FINAL) THEN
!-----------------------------------------------------------------------
      IF (CROP .NE. 'FA') THEN
...
!     Calculate harvest residue left in field
        CALL HRes_CGRO(CONTROL,SOILPROP,
     &    CROP, DWNOD, HARVFRAC, NLAYR, PLIGLF, PLIGNO,   !Input
...
! RHOL      Fraction of leaf which is carbohydrate (g [CH2O] / g[leaf])
