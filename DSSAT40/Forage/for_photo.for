...
C-----------------------------------------------------------------------
      ...
      PG = PG * EXCESS
       WRITE (6666,'(I10,5X,8(F10.6,6X))') YRDOY,PTSMAX,PG,TPGFAC,SWFAC,
     &      AGEFCC,PRATIO,SLPF,Tday
...
!-----------------------------------------------------------------------
      ...
      INTEGER II, PATHL, LUNCRP, ERR, LINC, ISECT, IPATH
...
c        CALL ERROR(ERRKEY, 1, FILEIO, LINC)
c      ELSE
c        READ(LUNIO,'(42X,F6.0)') ROWSPC
c      ENDIF
...
c        CALL ERROR(ERRKEY, 1, FILEIO, LINC)
c      ELSE
c        READ(LUNIO,'(60X,F6.0,6X,F6.0)') PHTHRS10, LMXSTD
c      ENDIF
