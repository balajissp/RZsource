...
!-----------------------------------------------------------------------
      ...
      CHARACTER*30 FILEIO
      CHARACTER*255 FILECC, FILEGC, PATHCR, PATHSR
      CHARACTER*78 MSG(2)
      CHARACTER*80 MOW80
...
C          PATHEC = CONTROL % PATHER
C          CROP = CONTROL % CROP
C          ECONO = PLANTVAR% ECONO
          WTPSD = PLANTVAR% WTPSD
C          SFDUR = PLANTVAR% SFDUR
C          SDPDVR = PLANTVAR% SDPDVR
C          PODUR = PLANTVAR% PODUR
...
C          LAI=REAL(ALAIRESET)
          ...
          XHLAI = REAL(ALAIRESET)   !Used in WATBAL
          AREALF = REAL(ALAIRESET) * 10000.
          SLA = AREALF / WTLF
       ENDIF
...
C     DIEGO ADDED DAILY SENESCENCE 11/22/2016
     ...
     &       .AND.ISECT.EQ.0)THEN
        ...
        ELSE
         write (*,*) 'MOW file has no data for this treatment', trtno
     ...
     &       .AND.ISECT.EQ.0)THEN
            I = I + 1
            READ (MOW80,'(I6,I8,1F6.0)',IOSTAT=ISECT)
     &                TRNO(I),DATE(I),MOW(I)
c            CALL Y2K_DOY(DATE(I))
          END IF
        END DO
...
!-----------------------------------------------------------------------
      ...
      CHARACTER*255 FILECC, FILEGC, PATHCR, PATHSR
      CHARACTER*255 C255
      CHARACTER*12 FILEC, FILES, FILEE
...
c        CALL ERROR(ERRKEY, 1, FILEIO, LNUM)
c      ELSE
c        READ (LUNIO,'(3X,A2)') CROP
c      ENDIF
...
c        CALL ERROR(ERRKEY, 1, FILEIO, LNUM)
c        ELSE
c        READ(LUNIO,'(24X,F6.1,5X,A1,6X,F6.0,12X,F6.0)')
c     &    PLTPOP, PLME, ROWSPC, SDWTPL
c        ENDIF
...
c        CALL ERROR(ERRKEY, 1, FILEIO, LNUM)
c        ELSE
c        READ(LUNIO,'(24X,A6,66X,F6.0)') ECONO, WTPSD
c        ENDIF
