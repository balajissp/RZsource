...
C---------------------------------------------------------              
     ...
     &                    pliglf, pligst
        ...
        END IF
c need to get alfalfa harvest from RZWQM2
c  MOW is residual biomass in kg/ha after mowing
c  RSPLF is residual percent leaf after mowing
c  MVS is the residual vegetative stage (node #) after mowing
c  RSHT is the residual height (cm) after mowing (not currently used)
        MOWLUN=999
...
C               WRITE (fhlun,fhoutfmt) mowfile(1:8),crop,trim(ename),
C     &             Version,VBranch,
C     &             MonthTxt(DATE_TIME(2)), DATE_TIME(3), DATE_TIME(1), 
