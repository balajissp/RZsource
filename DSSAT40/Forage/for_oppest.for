...
C  Calls:     None
C=======================================================================
      SUBROUTINE FOR_OPPEST(CONTROL, ISWITCH, 
...
C-------------------------------------------------------------------
      ...
      INTEGER DAP, DAS, DOY, DYNAMIC, ERRNUM, FROP, LUNIO, MULTI
...
C-----------------------------------------------------------------------
        ...
        IF (RNMODE .EQ. 'Q') THEN
        CALL HEADER(SEASINIT, FILEIO, NOUTDD, REPNO)
c        CALL HEADER(SEASINIT, NOUTDD, REPNO)
        ELSE
        CALL HEADER(SEASINIT, FILEIO, NOUTDD, RUN)
c        CALL HEADER(SEASINIT, NOUTDD, RUN)
        ENDIF

        WRITE (NOUTDD,2190)
 2190     FORMAT('@YEAR DOY   DAS   DAP  DLA   DLA% ',
     &   ' DLAI  DLFM  DSTM  DSDM  DSD#  DSHM  DSH#  DRTM  DRLV ',
     &   ' DRLF  DASM  DPO%  CLAI  CLFM  CSTM  CSDM  CSD#  CSHM ',
     $       ' CSH#  CRTM  CRLV  CRLF   CASM  CPO% ')
        ENDIF
...
C-----------------------------------------------------------------------
      IF (MOD(DAS,FROP) .EQ. 0) THEN
        ...
        WRITE (NOUTDD,300)YEAR, DOY, DAS, DAP,INT(DISLA),DISLAP,
     &  LAIDOT,WLIDOT,WSIDOT,SWIDOT,SDIDOT,WSHIDT,SHIDOT,WRIDOT,
     &  RLVDOT,RLFDOT,ASMDOT,PPLTD,
     &  CLAI,CLFM,CSTEM,CSDM,INT(CSDN),CSHM,INT(CSHN),CRTM,CRLV,
     &  CRLF,CASM,CPPLTD
300   FORMAT(1X,I4,1X,I3.3,3(1X,I5),6(1X,F5.1),1X,F5.2,1X,F5.1,
     &  5(1X,F5.2),1X,F5.1,3(1X,F5.0),2(1X,I5,1X,F5.1),
     &  2(1X,F5.1),1X,F6.1,1X,F5.1)
      ENDIF
...
C-----------------------------------------------------------------------
      ...
      IF (RNMODE .EQ. 'Q') THEN
        ...
        WRITE (NOUTDD,300)YEAR, DAY, 0, 0, 0, 0.0,
...
!***********************************************************************
      RETURN
      END !SUBROUTINE FOR_OPPEST
!-----------------------------------------------------------------------
!     FOR_OPPEST Variable Definitions
!-----------------------------------------------------------------------
...
! TITLET  Treatment description
! TRTNO   Treatment number being simulated by crop model
