...
C          PHTHRS10 = PLANTVAR% PHTHRS(10)
C          LMXSTD = PLANTVAR% LFMAX
...
c        CALL ERROR(ERRKEY, 1, FILEIO, LNUM)
c      ELSE
c        READ(LUNIO,'(24X,A6,48X,3F6.0)') ECONO, SLAVAR, SIZELF, XFRUIT
c      ENDIF
...
C          CALL GETPUT_PLANTVAR('GET',PLANTVAR)
           SLAVAR = PLANTVAR% SLAVAR
          SIZELF = PLANTVAR% SIZELF
          ECONO = PLANTVAR% ECONO
          XFRUIT = PLANTVAR% XFRUIT
	    PLME = PLANTVAR% PLME
	    SDAGPL = PLANTVAR% SDAGE
C          LMXSTD = PLANTVAR% LFMAX
C          ROWSPC = PLANTVAR% ROWSPC
C END OF MODIFICATION
      CALL GETLUN('FILEC', LUNCRP)
