...
!       REAL DUL(NL), LL(NL)
      ...
      EFINOC = PLANTVAR%EFINOC
      EFNFIX = PLANTVAR%EFNFIX 
...
c      ELSE
c        READ(LUNIO,'(28X,2F6.0)',IOSTAT=ERR) EFINOC, EFNFIX
c        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILEIO,LNUM)
c      ENDIF

c      IF (EFINOC .LE. 0.0) EFINOC = 1.0
c      IF (EFNFIX .LE. 0.0) EFNFIX = 1.0
