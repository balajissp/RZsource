...
C MODIFIED BY LIWANG MA, RZWQM-DSSAT      
C      OPEN (UNIT=LUNIO, FILE = FILEIO, STATUS = 'OLD', 
C     &  IOSTAT=ERRNUM)
...
C      ELSE
C        READ(LUNIO,'(3X,A8,1X,A2,1X,A60)',IOSTAT=ERRNUM) EXPER,CG,ENAME
C        LNUM = LNUM + 1
...
C        CALL ERROR(SECTION, 42, FILEIO, LNUM)
C      ELSE
C        READ (LUNIO,'(I3,I2,2(1X,I1),1X,A25)',IOSTAT=ERRNUM) 
C     &      TRTNO, ROTNO, ROTOPT, CRPNO, TITLET
C        LNUM = LNUM + 1
C        IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM)
C      ENDIF
C
...
C        CALL ERROR(SECTION, 42, FILEIO, LNUM)
C      ELSE
C        READ(LUNIO,'(3X,A2)',IOSTAT=ERRNUM) CROP
C        LNUM = LNUM + 1
C        IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM)
C      ENDIF
C
...
C        CALL ERROR(SECTION, 42, FILEIO, LNUM)
C      ELSE
C        READ(LUNIO,'(3X,A8,1X,A8,49X,A10)',IOSTAT=ERRNUM) 
C     &        FLDNAM, WSTA, SLNO
C        LNUM = LNUM + 1
C        IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEIO,LNUM)
C      ENDIF
...
!     & '  RECM   ONAM    OCAM  PI#M  PICM  PUPC  SPAM') !chp 11/29/2005
     ...
     &    CROP, TITLET(1:48), !FLDNAM, WSTA, SLNO,
...
!     &    NNAPHO, NINT(TOTPH), NINT(TOTPUP), NINT(TPLEFT) !, NUCMF
  ...
  500  FORMAT (I9,1X,I6,3(I3),
     &  1X,A2,1X,A48,   !1X,A8,1X,A8,1X,A10,
...
!     Write Evaluate.OUT file
     ...
     &  (CROP .NE. 'WH' .AND. CROP .NE. 'BA' .and. crop.ne.'TR')) THEN
