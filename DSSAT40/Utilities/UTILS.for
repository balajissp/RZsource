...
!-----------------------------------------------------------------------
      ...
      LOGICAL FEXIST, FPRINT(300)
      INTEGER COUNTER, Length, I, OUTLUN, StartLun
      CHARACTER*30 SaveName(300)
c LIWANG MA, 90 TO 190, 83 TO 183, 200 to 300 for Fprint and Savename
      DATA StartLun /90/
      DATA COUNTER /0/
      DATA FPRINT /300*.FALSE./
      DATA OUTLUN /183/    !List.OUT - list of unit assignments
...
C  MODIFIED FOR RZWQM-DSSAT BY Liwang Ma
C  SO THAT THE FILE NUMBER WILL NOT REPEAT RZWQM
      LUN = LUN +100
...
!-----------------------------------------------------------------------
      ...
      logical fexist, fopened, ERR, FIRST, EOF
      integer unumber
...
!       and FILEIO.
     ...
     &        IOSTAT = ERRNUM)
        IF (ERRNUM .EQ. 0) THEN
          I = 0
          !DO WHILE (.NOT. EOF (LUNSRC))
          DO ! RM Added end of file handling to the read.
            I = I + 1
            READ(LUNSRC,'(A80)',END=50,ERR=50) HEADER1(I)
          ENDDO
   50     CONTINUE
...
C
C        if (fexist) then
C          inquire (file = fileio, opened = fopened)
C          if (fopened) then
C            inquire (file = fileio, number = unumber)
C            close(unumber)
C          endif
C
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
          ERR = .FALSE.
C          OPEN (UNIT=LUNIO, FILE=FILEIO, STATUS='OLD', IOSTAT=ERRNUM)
C          IF (ERRNUM .NE. 0) THEN
C            ERR = .TRUE.
C            GOTO 150
C          ENDIF
...
C-----------------------------------------------------------------------
C          READ (LUNIO,'(//, 15X,A8)',IOSTAT=ERRNUM) MODEL
C          LNUM = LNUM + 3
C          IF (ERRNUM .NE. 0) ERR = .TRUE.
...
C            ERR = .TRUE.
C          ELSE
C            READ(LUNIO, 120,IOSTAT=ERRNUM) EXPER, ENAME; LNUM = LNUM + 1
C  120       FORMAT(3X,A8,4X,A60)
C            IF (ERRNUM .NE. 0) ERR = .TRUE.
C          ENDIF
C-----------------------------------------------------------------------
...
C            ERR = .TRUE.
C          ELSE
C            READ(LUNIO, 130,IOSTAT=ERRNUM) TRTNO, TITLET; LNUM = LNUM +1
C  130       FORMAT(I3,6X,A25)
C
C            IF (ERRNUM .NE. 0) ERR = .TRUE.
C          ENDIF
...
C            ERR = .TRUE.
C          ELSE
C            READ(LUNIO,140,IOSTAT=ERRNUM) CROP ; LNUM = LNUM + 1
C  140       FORMAT(3X,A2)
C            IF (ERRNUM .NE. 0) ERR = .TRUE.
C          ENDIF
...
!       Last, check for file in C:\DSSAT4 directory
      ...
      IF (FEXIST) THEN
        ...
        DO
          !RM @ end of file: jump to 999 
          READ(LUN,'(A2,7X,A30)',END=999, IOSTAT=ERR) CROPID, CRDESC
          LNUM = LNUM + 1
...
!      CASE ('MO'); CROPD = 'MILLET-PRO'
!      CASE ('MX'); CROPD = 'MILLET-FOX'
!      CASE ('TR'); CROPD = 'TRITICALE '
!      CASE ('CA'); CROPD = 'CANOLA    '
!      CASE ('PE'); CROPD = 'PEA       '
