...
c      IF (MEEVP .EQ. 'D' .AND. INDEX('RIWHMLMZSGPTBA',CROP) .GT. 0) THEN
cc      IF (INDEX('RIWHMLMZSGPTBAMOMXTRSF',CROP) .GT. 0) THEN
...
!     Potato 
     ...
     &    LFWT,STMWT,XSTAGE,DTT,BIOMAS,SDWT,GRNWT,    !OUTPUT
     &    RTWT,STOVWT,STOVN,ROOTN,GRAINN,RTDEP,             !Output
     &    ISTAGE,TRNU,SWFAC,turfac)           
...
!     Rice 
C      ELSEIF (INDEX(MODEL,'RICER') .GT. 0) THEN
...
C
C        IF (CONTROL%DYNAMIC .EQ. INTEGR) THEN
C          XHLAI = XLAI
C        ENDIF
C
!     Potato 
c      IF (INDEX(MODEL,'PTSUB') > 0) THEN 
c        CALL SUBSTOR(CONTROL, ISWITCH, 
...
!     Maize 
c      ELSEIF (INDEX(MODEL,'MZCER') .GT. 0) THEN
c        CALL MZ_CERES (CONTROL, ISWITCH,                  !Input
c     &    CO2, DAYL, EOP, HARVFRAC, NH4, NO3, SNOW,       !Input
c     &    SOILPROP, SRAD, SW, TMAX, TMIN, TRWUP,          !Input
c     &    TWILEN, YREND, YRPLT,                           !Input
...
!     Sorghum 
c      ELSEIF (INDEX(MODEL,'SGCER') .GT. 0) THEN
...
!     Millet 
c      ELSEIF (INDEX(MODEL,'MLCER') .GT. 0) THEN
...
!     Wheat and Barley 
c      ELSEIF (INDEX(MODEL,'WHCER') .GT. 0) THEN
c        CALL CSCERES_Interface (CONTROL, ISWITCH,          !Input
c     &     CO2, DAYL, EOP, YREND, NH4, NO3, SNOW, SOILPROP,!Input
c     &     SRAD, SRFTEMP, ST, SW, TMAX, TMIN, TRWUP, TWILEN,!Input
