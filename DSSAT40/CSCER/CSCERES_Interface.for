...
C=======================================================================
     ...
     &     RLV, RWUMX, SENESCE, STGDOY, UNH4, UNO3, XLAI,
     &     GRWT,LFWT,STWT,RTWT,ISTAGE,NUPD,GNAD,CNAD,ROOTN,
     &     WFG,RTDEP,TFG,CWAD,RTWTS,wflf)  !Output
      ...
      INTEGER STGDOY(20), YEARPLT, ISTAGE
      ...
      REAL GRWT,LFWT,STWT,RTWT,NUPD,GNAD,CNAD,ROOTN,WFG
	REAL RTDEP,TFG,CWAD,RTWTS,pltpop,wflf                                     !LIWANG MA
...
!     Print warning if Century soil N routine used
  ...
  120 FORMAT('use with this model.' )
      ...
      IF (DYNAMIC .EQ. RUNINIT .OR. DYNAMIC .EQ. SEASINIT) THEN
        ...
        SN = 0
        ON = 0
        CN = 1  !Crop component changed to 1 by liwang ma, rzwqm-dssat
        REP = 1
        STEP = 1
        RUNI = 1
...
C-----------------------------------------------------------------------
     ...
     & STGDOY,                                             !Stage dates
     & DYNAMIC,                                            !Control 
     & ISWITCH%IPLTI,CONTROL%YRPLT,CONTROL%CROP,PLANTVAR%VARNO, !LIWANG MA, RZWQM-DSSAT
     & PLANTVAR%VRNAME,PLANTVAR%PLTPOP,PLANTVAR%ROWSPC,   !LIWANG MA
     & PLANTVAR%SDEPTH,CONTROL%ENAME,CONTROL%EXPER//CONTROL%CROP,
     & CONTROL%TITLET,PLANTVAR%HDATE(1),ISWITCH%IHARI,
     & ISWITCH%IFERI,PLANTVAR%HPC,CONTROL%FILEC,CONTROL%PATHCR,
     & CONTROL%FILEE,CONTROL%PATHER,CONTROL%FILES,
     & CONTROL%PATHSR,PLANTVAR%ECONO,PLANTVAR%P1V,PLANTVAR%P1D,
     & PLANTVAR%P5,PLANTVAR%G1,PLANTVAR%G2,PLANTVAR%G3,
     & PLANTVAR%PHINT,GRWT,LFWT,STWT,RTWT,ISTAGE,NUPD,GNAD,CNAD,
     & ROOTN,WFG,RTDEP,TFG,CWAD,RTWTS,CONTROL%FILEA,CONTROL%FILET,
     & control%yrsim,PLANTVAR%HBPC,plantvar%pltpop,wflf)                                               !LIWANG MA
