...
ifeq ($(ENV),ifort)
 F77 = ifort
 F77_FLAGGED=$(F77) -nologo -fpp -shared-intel -save -zero -module $(ODIR)/ -Fo$(ODIR)/ -I$(ODIR)/ -module $(ODIR)/ -threads -f77rtl
 F77_LINKER=$(F77) -threads
 ODIR = ./IntelBuild
else
 F77 = gfortran
...
.PHONY: all installRZlinux install cleanODIR clean distclean

all: $(ODIR) $(ODIR)/RZlinux

installRZlinux:
	mv $(ODIR)/RZlinux ../bin

install:
	. /etc/environ.sh; use -e -r intel-20220102; make ENV=ifort all installRZlinux

cleanODIR: 
	rm -rf $(ODIR)

clean: 
	. /etc/environ.sh; use -e -r intel-20220102; make ENV=ifort cleanODIR
