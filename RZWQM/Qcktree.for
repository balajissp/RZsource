...
C
      SUBROUTINE QUICKTREE(FIRST1,JDAY,NN,CN,RDF,HEIGHT,LAI,TLAI,
     +    PLTSLV,PWRTS,DMDNIT,RM,RCN,RPOOL,PLDEN,JGS,TM,PLNAME,TLT,TL,
     +    ISDP1,TADRT,TADRTC,iyyy,OMSEA,RCO2,resage)
...
C=============================================================
      IF(FIRST1) THEN
        FIRST1=.FALSE.
        OPEN(INP2,FILE='QCKTREE.DAT',STATUS='OLD')
        IDX=0
        CALL ECHO(INP2)
        READ(INP2,*) NPL
        DO 10 IP=1,NPL
...
c        HEIGHT=THEIGHT/RUN*IGDAY
        ...
        LAI=MIN(LAI,TOTLAI)
        TLAI=LAI
        HEIGHT=theight  !keep height constant
c        HEIGHT=HEIGHT+prop(igday)*theight
c        HEIGHT=MIN(HEIGHT,THEIGHT)
c Liwang Ma
C       
C=================================================================
...
C           ..ADD TO SOIL RESIDUE POOLS
            DO 120 I=1,NN
c              IF (ROOTMASS.EQ.0.0D0) THEN
c                 RMASS=FODDER*ROOTSHT*RDF(I)  !only the specified dead root into the residue pools
c              ELSE 
...
C     
 ...
 1100 FORMAT(70('*'))
 1200 FORMAT(/'Quick Tree End of Growing Season On ',
     +      4('-'),I3,'/',I2,'/',I4,' -----',I6,'  ----'/)
      END
