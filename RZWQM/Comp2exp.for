...
C====================================================================== 
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       ...
       DIMENSION PDATA(25,mxnod),EAVGHZ(1700)  !,EAVGTM(3000)
       DIMENSION ZHOR(12),EXPD(25,1700,maxhor),DEP(25,1700,maxhor)
       Dimension AVGS(25,1700,maxhor),RMSE(1700),!DIFF(1200),
     +           IJDAY(25,1700),ipcrop(100)
       DIMENSION IDAY(25,1700),IMONTH(25,1700),IYEAR(25,1700)
       DIMENSION IDEXP(25),NEDATA(25),SAVGHZ(1700)   !,SAVGTM(3000)
       DIMENSION JNAME(26), MHORZ(25,1700),TLT(mxnod),PRED1(mxnod)
       DIMENSION DS(mxnod),SOILP(25,1700,mxnod),pred2(mxnod),dsk(mxnod),
     ...
     +           EBGBIOM(100),DAYBGBIOM(100),PBGBIOM(100),IBBGCROP(100)
     ...
     +           EP2ROF(100),PP2ROF(100),EP3ROF(100),PP3ROF(100)
       DIMENSION PPHT(500),EPHT(500),PAET(1000),EAET(1000),DAYPHEN(100),
     +           DAYPHT(500),DAYETS(1000),DAYETE(1000),ELAI(500),
     ...
     +           edap(100),EEPHEN(100),ep3year(100),pp3year(100),
     +           ep1year(100),pp1year(100),ep2year(100),pp2year(100), 
     +           eaccp1(1200),paccp1(1200),eaccp2(1200),paccp2(1200),
     +           eaccp3(1200),paccp3(1200),ptileP1mon(100,12),
     +           ptileP2mon(100,12),ptileP3mon(100,12),indbgbiom(100)
       CHARACTER*80 TITLE
...
C
     ...
     &         ,'Aug','Sep','Oct','Nov','Dec'/
       data plant63/0.0d0/,plant41/0.0d0/,plant42/0.0d0/,plant62/0.0d0/,
     &      plant44/0.0d0/,rmseno3/0.0d0/,plant43/0.0d0/,
     &      plant21/0.0d0/,plant22/0.0d0/,plant23/0.0d0/,plant24/0.0d0/
C
        OPEN (UNIT=1,FILE='LAYER.PLT',STATUS='OLD',err=13) 
        OPEN (UNIT=3,FILE='expdata.dat',STATUS='OLD',err=23) ! RM - name change for linux case sensitive
...
C READ ANALYSIS FILE
        ...
        OPEN(UNIT=11, FILE='Phenology.out', STATUS='UNKNOWN')  ! RM - name change for linux case sensitive
...
C
      ...
      else if (knumb1.eq.2) then
        READ (UNIT=3,FMT=*,err=27) NEDATA(II), ITYPESWC
      else if (knumb1.eq.3) then
        READ (UNIT=3,FMT=*,err=27) NEDATA(II), ITYPESWC,iresetswc
      endif
...
C  READ PLANT HEIGHT (CM)
      ...
      backspace(3)      
      if (knumb4.eq.1) then
      READ (UNIT=3, FMT=*,err=36) NPHT  !maximum 200
      else if (knumb4.eq.2) then
      READ (UNIT=3, FMT=*,err=36) NPHT,iresetht  !maximum 200
      endif
        if (npht.gt.500) then
...
C  READ ET MEASUREMENTS (CM)
      ...
      CALL ECHO(3)
      READ (UNIT=3, FMT=*,err=36) NET   !maximum 200
        if (net.gt.1000) then
        print*, 'Number of ET observations exceed 200'
        stop
        endif
      DO 87 K=1,NET
...
C  READ LAI MEASUREMENTS (CM)
      ...
      READ (UNIT=3, FMT=*,err=36) NLAI,iresetlai  !maximum 200
      endif      
        if (nlai.gt.500) then
        print*, 'Number of LAI observations exceed 500'
        stop
        endif
      DO 97 K=1,NLAI
      if (knumb4.eq.1) then
      ...
      READ (UNIT=3,FMT=*,err=36) IDLAI,IMLAI,IYLAI,ELAI(K),idaylai
      endif
...
C  READ phenology MEASUREMENTS, anthesis and physiological maturity dates (DAP)
     ...
     +                DBLE(JDATE(IDPHEN,IMPHEN,IYPHEN))
      ENDDO
      ENDIF
C  READ PLANT BG BIOMASS DATA
      IF (ISDATA(12).EQ.1) THEN
      CALL ECHO(3)
      READ (UNIT=3, FMT=*,err=36) NBGBIOM               !maximum 100
        if (nbgbiom.gt.100) then
        print*, 'Number of biomass observations exceed 100'
        stop
        endif
      DO K=1,NBGBIOM
      READ (UNIT=3,FMT=*,err=36)IDBGBIOM,IMBGBIOM,IYBGBIOM,EBGBIOM(K),
     +   IBBGCROP(K)
         DAYBGBIOM(K) = DBLE(IYBGBIOM)+DBLE(JDATE(IDBGBIOM,
     +                  IMBGBIOM,IYBGBIOM))*1.0D-3
        indbgbiom(k)=iybiom
      ENDDO
      ENDIF
C
...
C
      ...
      READ (UNIT=5,FMT=*,err=28,end=66) (ANADATA(I),I=1,MXANA)
          ...
          plant21=max(plant21,anadata(21))
          plant22=max(plant22,anadata(22))
          plant23=max(plant23,anadata(23))
          plant24=max(plant24,anadata(24))
...
61    CONTINUE
C
C  CALCULATE BELOWGROUND BIOMASS FROM ANALYSIS FILE
      DO I=1,NBGBIOM
        IF (DAYBGBIOM(I).EQ.ANADATA(1).AND.   !) THEN
     +      DAYBGBIOM(I).GE.DAYB.AND.DAYBGBIOM(I).LE.DAYE) THEN
            IF (ANADATA(42).GT.0.0D0) THEN
            PBGBIOM(I)= ANADATA(42)
            ELSE
            PBGBIOM(I)= max(ANADATA(42),plant42)
            plant42=0.0d0
            ENDIF
            RMSEBGBM = RMSEBGBM + (EBGBIOM(I)-PBGBIOM(I))**2
        ENDIF
       ENDDO
C
C  CALCULATE N UPTAKE BY ABOVEGROUND BIOMASS FROM ANALYSIS FILE
     ...
     +      DAYNUP(I).GE.DAYB.AND.DAYNUP(I).LE.DAYE) THEN
            IF (ANADATA(22).GT.0.0D0) THEN
            PNUP(I)= ANADATA(22)
            ELSE
            PNUP(I) = max(ANADATA(22),plant22)
            plant22=0.0d0
            ENDIF
...
C  CALCULATE N IN GRAIN FROM ANALYSIS FILE
     ...
     +      DAYGN(I).GE.DAYB.AND.DAYGN(I).LE.DAYE) THEN
            IF (ANADATA(24).GT.0.0D0) THEN
            PGRAINN(I)= ANADATA(24)
            ELSE
            PGRAINN(I) = max(ANADATA(24),plant24)
            plant24=0.0d0
            ENDIF
...
C
     ...
     +           INT(ANADATA(1)))
            ...
            PTILEP1mon(Iyi,imi)= PTILEP1mon(Iyi,imi)+ANADATA(48)    
            PTILEP2mon(Iyi,imi)= PTILEP2mon(Iyi,imi)+ANADATA(53)    
            PTILEP3mon(Iyi,imi)= PTILEP3mon(Iyi,imi)+ANADATA(58)    
c
            write(cday,'(i2.2)') idi ! RM removed comma after end paren.
            write(cyr,'(i4.4)') int(anadata(1)) ! RM removed comma after end paren.
            write(cmon,'(i2.2)') imi ! RM removed comma after end paren.
...
c insert missing P1 data
      if (ep1ppm(ijk).lt.0.0d0) then
	    eac1=-99
	    eac2=-99
          do ijkl=ijk-1,1,-1
          if (ep1ppm(ijkl).ge.0.0d0) then
		    eac1=ep1ppm(ijkl)
	        goto 4234
	    endif
          enddo
...
5134   continue
          ...
          ep1ppm(ijk)=(eac1 + eac2)/2
	    endif
      endif
          if (ep1ppm(ijk).lt.0.0d0) ep1ppm(ijk)=0.0d0
c end of inserting missing P1 data
c insert missing P2 data
      if (ep2ppm(ijk).lt.0.0d0) then
	    eac1=-99
	    eac2=-99
          do ijkl=ijk-1,1,-1
          if (ep2ppm(ijkl).ge.0.0d0) then
		    eac1=ep2ppm(ijkl)
	        goto 2234
	    endif
          enddo
...
3134   continue
          ...
          ep2ppm(ijk)=(eac1 + eac2)/2
	    endif
      endif
          if (ep2ppm(ijk).lt.0.0d0) ep2ppm(ijk)=0.0d0
c end of inserting missing P2 data
c insert missing P3 data
      if (ep3ppm(ijk).lt.0.0d0) then
	    eac1=-99
	    eac2=-99
          do ijkl=ijk-1,1,-1
          if (ep3ppm(ijkl).ge.0.0d0) then
		    eac1=ep3ppm(ijkl)
	        goto 3234
	    endif
          enddo
...
4134   continue
          ...
          ep3ppm(ijk)=(eac1 + eac2)/2
	    endif
      endif
          if (ep3ppm(ijk).lt.0.0d0) ep3ppm(ijk)=0.0d0
c end of inserting missing P3 data
c from ppm to kg/ha
...
c from ppm to kg/ha
      if (ep1ppm(ijk).ge.0.0d0) then
             ep1year(ijkyear) = ep1year(ijkyear)+
     +                      etilefd(ijk)*ndays(ijk)*ep1ppm(ijk)/10.0d0
             pp1year(ijkyear) = pp1year(ijkyear)+
     +                      ptilef(ijk)*ndays(ijk)*ptilep1(ijk)/10.0d0
             EACCp1(ijk) =  eaccp1(ijk-1)+
     +                      etilefd(ijk)*ndays(ijk)*ep1ppm(ijk)/10.0d0
             PACCp1(ijk) =  paccp1(ijk-1)+
     +                      ptilef(ijk)*ndays(ijk)*ptilep1(ijk)/10.0d0
      endif
C
c from ppm to kg/ha
      if (ep2ppm(ijk).ge.0.0d0) then
             ep2year(ijkyear) = ep2year(ijkyear)+
     +                      etilefd(ijk)*ndays(ijk)*ep2ppm(ijk)/10.0d0
             pp2year(ijkyear) = pp2year(ijkyear)+
     +                      ptilef(ijk)*ndays(ijk)*ptilep2(ijk)/10.0d0
             EACCp2(ijk) =  eaccp2(ijk-1)+
     +                      etilefd(ijk)*ndays(ijk)*ep2ppm(ijk)/10.0d0
             PACCp2(ijk) =  paccp2(ijk-1)+
     +                      ptilef(ijk)*ndays(ijk)*ptilep2(ijk)/10.0d0
      endif
C
c from ppm to kg/ha
      if (ep3ppm(ijk).ge.0.0d0) then
             ep3year(ijkyear) = ep3year(ijkyear)+
     +                      etilefd(ijk)*ndays(ijk)*ep3ppm(ijk)/10.0d0
             pp3year(ijkyear) = pp3year(ijkyear)+
     +                      ptilef(ijk)*ndays(ijk)*ptilep3(ijk)/10.0d0
             EACCp3(ijk) =  eaccp3(ijk-1)+
     +                      etilefd(ijk)*ndays(ijk)*ep3ppm(ijk)/10.0d0
             PACCp3(ijk) =  paccp3(ijk-1)+
     +                      ptilef(ijk)*ndays(ijk)*ptilep3(ijk)/10.0d0
      endif
C
...
c
   71 CONTINUE
      IF (NTF.GT.0) THEN
      CALL STATS(ETILEFD,PTILEF,NTF,
     & RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=574) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      WRITE (UNIT=6,FMT=573) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      RMSETF = RMSESTAT
      ELSE
...
c         WRITE (UNIT=4,FMT=120)
      ...
      ENDIF
      IF (NTFP1.GT.0) THEN
      CALL STATS(EP1PPM,PTILEP1,NTF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c         WRITE (UNIT=4,FMT=120)
      ...
      ENDIF
      IF (NTFP2.GT.0) THEN
      CALL STATS(EP2PPM,PTILEP2,NTF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c         WRITE (UNIT=4,FMT=120)
      ...
      ENDIF
      IF (NTFP3.GT.0) THEN
      CALL STATS(EP3PPM,PTILEP3,NTF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c      endif
     ...
     +      PWTB(I)
      ...
      ENDIF
   78 CONTINUE
      CALL STATS(EWTB,PWTB,NWTB,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c      endif
   ...
   79 CONTINUE
      IF (NROF.GT.0) THEN
      CALL STATS(EROF,PROF,NROF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c         WRITE (UNIT=4,FMT=120)
      ...
      ENDIF
      IF (NROFN0.GT.0) THEN
      CALL STATS(ENROF,PNROF,NROF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c         WRITE (UNIT=4,FMT=120)
      ...
      ENDIF
      IF (NROFP1.GT.0) THEN
      CALL STATS(EP1ROF,PP1ROF,NROF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c         WRITE (UNIT=4,FMT=120)
      ...
      ENDIF
      IF (NROFP2.GT.0) THEN
      CALL STATS(EP2ROF,PP2ROF,NROF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c         WRITE (UNIT=4,FMT=120)
      ...
      ENDIF
      IF (NROFP3.GT.0) THEN
      CALL STATS(EP3ROF,PP3ROF,NROF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
c      endif
     ...
     +      PPHT(I)
      ENDIF
   88 CONTINUE
      if (iresetht.le.0) then
      CALL STATS(EPHT,PPHT,NPHT,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      RMSEPHT = RMSESTAT
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      else
       RMSEPHT = 0.0d0
      WRITE (UNIT=6,FMT=174) 0.0d0
      ENDIF
      endif
C
      IF (ISDATA(9).EQ.1) THEN
...
c      WRITE(UNIT=4,FMT=780)
      ...
      DO 98 I=1,NET
      IF(DAYETS(I).GE.DAYB.AND.DAYETE(I).LE.DAYE) THEN
...
c      endif
     ...
     +                        EAET(I),PAET(I)
      Endif
   98 CONTINUE
      CALL STATS(EAET,PAET,NET,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
...
c      endif
     ...
     +      PLAI(I)
      ENDIF
   83 CONTINUE
      IF (IRESETLAI.LE.0) THEN
       CALL STATS(ELAI,PLAI,NLAI,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      RMSELAI = RMSESTAT
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      ELSE
...
c      endif
      ENDIF
   74 CONTINUE
      CALL STATS(EYIELD,PYIELD,NYIELD, 
     & RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
...
c      endif
     ...
     +                phindex
      ENDIF
   73 CONTINUE
      CALL STATS(EBIOM,PBIOM,NBIOM,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
...
c      WRITE(UNIT=4,FMT=774)
        WRITE (UNIT=6,FMT=121)
      WRITE(UNIT=6,FMT=784)
      DO I=1,NBGBIOM
      IF(DAYBGBIOM(I).GE.DAYB.AND.DAYBGBIOM(I).LE.DAYE) THEN
      CALL CDATE(NINT((DAYBGBIOM(I)-INT(DAYBGBIOM(I)))*1000),ID,IM,
     +           INT(DAYBGBIOM(I)))
      WRITE (UNIT=6, FMT=260) ID,IM,INT(DAYBGBIOM(I)),
     +   NINT((DAYBGBIOM(I)-INT(DAYBGBIOM(I)))*1000),EBGBIOM(I),
     +      PBGBIOM(I),IBBGCROP(I)
      ENDIF
      ENDDO
      CALL STATS(EBGBIOM,PBGBIOM,NBGBIOM,RSQ,SLOPE,SINTER,RMSESTAT,
     &          TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      RMSEBGBM = RMSESTAT
      ENDIF
C
...
c      endif
     ...
     +      phindex*100
      ENDIF
   72 CONTINUE
      CALL STATS(ENUP,PNUP,NNUP,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
...
c      endif
     ...
     +      phindex*100
      ENDIF
  109 CONTINUE
      CALL STATS(EGRAINN,PGRAINN,NGRAINN,RSQ,SLOPE,SINTER,
     *     RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      WRITE (UNIT=6,FMT=174) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
...
C
                  ...
                  SOILP(ii,i,IJ) = pdata(jname(26),ij)
                  endif
122               CONTINUE
...
c     &         .or.(ii.eq.17).or.(ii.eq.19)) then
            if ((ii.ne.1).and.(ii.ne.10).and.(ii.ne.13).and.(ii.ne.16)
     &         .and.(ii.ne.18).and.(ii.ne.20).and.(ii.ne.21).and.
     &         (ii.ne.22).and.(ii.ne.23).and.(ii.ne.24))
     &         then
...
c     +               DS(J),SOILPs(J),EXPD(II,I,J),AVGS(II,I,J)
     ...
     +            ,JULDATE(IDAY(II,I),IMONTH(II,I)
     +                       ,IYEAR(II,I))-JULDATE(31,12
     +                       ,IYEAR(II,I)-1),
...
750         CONTINUE
C
      IF (IRESETSWC.EQ.1.and.ii.eq.1) THEN
      RMSESWC = 0.0d0
      WRITE (UNIT=6,FMT=174) 0.0D0
      ELSE
      CALL STATS(EAVGHZ,SAVGHZ,NEDATA(II),RSQ,SLOPE,
     &        SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
...
567   continue
      CALL STATS(ETfyear,PTfyear,NTFy,
     & RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
c      WRITE (UNIT=4,FMT=574) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
...
c      endif
     ...
     &,ep1year(i),pp1year(i),ep2year(i),pp2year(i),ep3year(i),pp3year(i)
568   continue
      CALL STATS(ETnyear,PTnyear,NTFny,RSQ,
     +           SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
      WRITE (UNIT=6,FMT=574) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      CALL STATS(Ep1year,Pp1year,NTFny,RSQ,
     +           SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
      WRITE (UNIT=6,FMT=574) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      CALL STATS(Ep2year,Pp2year,NTFny,RSQ,
     +           SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
      WRITE (UNIT=6,FMT=574) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      CALL STATS(Ep3year,Pp3year,NTFny,RSQ,
     +           SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
      WRITE (UNIT=6,FMT=574) RMSESTAT,RSQ,SLOPE,SINTER,TTEST,T5
      endif
...
c      endif
c      enddo
c      CALL STATS(EYIELD,PYIELD,NYIELD,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5)
...
c      endif
     ...
     +      PACCTF(I),EACCTN(I), PACCTN(I),EACCp1(I), PACCp1(I)
     +     ,EACCp2(I), PACCp2(I),EACCp3(I), PACCp3(I)
...
cc     +                        RMSESWC,RMSENO3,RMSENB,RMSENG
      IF (NTF.GT.0) THEN
      CALL STATS(EACCTF,PACCTF,NTF,RSQ,SLOPE,SINTER,RMSESTAT,TTEST,T5) ! RM Stat = intrinsic file funct, replaced with stats.
      RMSEACCTF=RMSESTAT 
      ENDIF
        WRITE (UNIT=6,FMT=121)
      WRITE (UNIT=6, FMT=271) RMSEYD,RMSEBM,RMSEBGBM,RMSELAI,RMSEPHT,
     +   RMSESWC,RMSESWCL,RMSEWT,RMSETF,RMSETN,RMSENO3,RMSENB,RMSENG,
     +   RMSEACCTF
      IF (ISDATA(5).EQ.1) THEN
...
c write out monthly tile flow and N in tile flow
     ...
     &        ptileP1mon(ijkl,ijkk),ptileP2mon(ijkl,ijkk),
     &        ptileP3mon(ijkl,ijkk)
         enddo
       write (unit=6,fmt=120)
       enddo  
      endif
389   format (/9x,'Year     Month       Monthly_tile  
     &  Monthly_N_tile   Monthly_P1_tile    Monthly_P2_tile  
     &   Monthly_P3_tile')
390   format (9x,i4,5x,i3,9(5x,G15.6))	 	       


103   FORMAT ("RZWQM2 Version 4.2 Compare to Experimental Results",/
     &    'Intel Fortran',
...
271   FORMAT (/13X,'ROOT MEAN SQUARE ERRORS FOR',13X,//'  YIELD  
     +   BIOMASS     BG BIOMSS       LAI            Plant_HT        
     + P-SWC          Layer-SWC       WATER-TAB        TILE-FL        
     + N-TILE        SOIL-NO3-N        N-BIOM         N-GRAIN        
     +  ACC-TF', /2(F9.2,1X),15(G15.6,1X))
...
252   FORMAT (/'TOTAL YEARLY TILE FLOW (CM)',//
     +          13x, '  YEAR   ',3x,'EXP-TF',3x,'     PRED-TF'/)
253   FORMAT (/'TOTAL YEARLY N loss to TILE FLOW (kg-N/ha)',//
     +           13x,'  YEAR   ','    EXP-N','        PRED-N',
     + '        EXP-P1','        PRED-P1','          EXP-P2',
     + '          PRED-P2','        EXP-P3','          PRED-P3'/)
...
733   FORMAT (/'TILE FLOW (CM) AND TILE N KG/HA',//,
     +           'DD MM YYYY  JULIAN       M-TILE       S-TILE
     +       M-N LOSS       S-N LOSS        M-P1 LOSS       S-P1 LOSS   
     +  M-P2 LOSS      S-P2 LOSS      M-P3 LOSS      S-P3 LOSS'/)
...
774   FORMAT (//'PLANT BIOMASS (KG/HA) ',//,
     ...
     +    M-Hindex     S-Hindex'/)
784   FORMAT (//'PLANT BG BIOMASS (KG/HA) ',//,
     +           'DD MM YYYY  JULIAN     M-BIOM         S-BIOM     Crop 
     +    M-Hindex     S-Hindex'/)
...
C
      SUBROUTINE STATS(YY,YYP,NOB,RSQ,SLOPE,SINTER,RMSE,TTEST,TVAR) ! RM Stat = intrinsic file funct, replaced with stats.
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
