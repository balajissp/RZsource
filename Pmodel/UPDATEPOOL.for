      SUBROUTINE UPDATEPOOL(ID,IM,IYYY)
C================================================================================
...
C===============================================================================  
      ...
      DO i = 1, Ncrop
        IF(Day==Chday(i).AND.Month==Chmon(i).AND.Year==Chyear(i)) THEN
          Avgrres(i) = Avgrbiom - Yeild
          Crpresleft(i) = Avgrres(i) + Blgrbiom
       IF (Avgrbiom + Blgrbiom> 0) THEN
       
          BlgrP = (Biopact(i)*Blgrbiom)/  
     +     (Avgrbiom + Blgrbiom)
          AvgrP = (Biopact(i)*
     +         (Avgrbiom -Yeild))/  
     +           (Avgrbiom + Blgrbiom)
          GrainP = (Biopact(i)*Yeild)/  
     +           (Avgrbiom + Blgrbiom)
        ELSE
           BlgrP =0.0
           AvgrP =0.0
       ...
       IF(Rootd > 0.0) THEN
          ...
          DO k = 1,j-1
            Ratio = Tsoil(k)/Rootd
            Blgrbiomly(k) = Blgrbiom*ratio
            BlgrPly(k) = BlgrP * Ratio
          END DO
            ratio = (Rootd - Dsoil(j-1))
     +                              /Rootd
            Blgrbiomly(j) = Blgrbiom*ratio
            BlgrPly(j) = BlgrP * Ratio
         
          
         DO k=1,J 
             CProot(k) = (Blgrbiomly(k)*0.4)/ (BlgrPly(k)+0.001)
